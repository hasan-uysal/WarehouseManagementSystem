...
namespace WarehouseManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT orderid, odate, O.pid, P.pname, O.fid, F.fname, qty, price, total FROM tbOrder AS O JOIN tbFactory AS F ON O.fid=F.fid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT (orderid, odate, O.pid, P.pname, O.fid, F.fname) LIKE '%"+txtSearch.Text+"%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
